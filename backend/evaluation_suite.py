...
REPORT_FILE = os.path.join(os.path.dirname(__file__), "..", "data", "benchmark_report.json")
...
def run_evaluation(model_names: list):
    ...
    for model in model_names:
        ...
        for task in EVAL_TASKS:
            ...
            if "error" in res:
                logging.error(f"Error evaluating {model} on {task['id']}: {res['error']}")
                continue
...
def run_custom_evaluation(model_names: list, custom_prompt: str):
    ...
    for model in model_names:
        ...
        model_metrics = {
            "model": model,
            "tasks": [{ "task_id": "custom", "latency": res["response_time"], "tps": res["tokens_per_sec"], "response": res["response"] }],
...
def generate_report_content(format: str):
    ...
    if format == "csv":
        ...
        rows = []
        for r in data:
            rows.append(f"{r['model']},{r['avg_tps']},{r['avg_latency']},{r['total_tokens']},{r['score']},{r['avg_cpu']},{r['avg_ram']}")
        return header + "\n".join(rows)
